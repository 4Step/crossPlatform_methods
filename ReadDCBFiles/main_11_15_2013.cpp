...
int main()
{
	...
	if(dictFile.is_open()){
       ...
       // For some reason, the file pointer was not able to set to the beigning
       dictFile.close();
       dictFile.open(ifileName);
       ifstream::pos_type size;
       size =  dictFile.tellg();
       cout << "At the end of file " <<  size <<"Total Lines: " << count_id << endl;

       // Set the position back to start
       // dictFile.seekg(7, ios::beg);
       size =  dictFile.tellg();
       cout << size << endl;
       ...
       cout<< "size is: "<< sizeLine << endl;
        ...
        for(int i=3; i< count_id; i++) {
          ...
          int nth_fields = fieldNames.size();
